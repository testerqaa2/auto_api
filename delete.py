...
class delete:
    ...
    def test_update_user(self):
        ...
        with sync_playwright() as p:
            ...
            try:
                ...
                self.updated_user_data = new_user_data.copy()
    ...
    def test_delete_user(self):
        if not self.created_user_id:
            print("❌ Can't run delete user - No user ID available from create test case")
            return False
        ...
        with sync_playwright() as p:
            ...
            try:
                print(f"3. Deleting user ID: {self.created_user_id} \n")
                print(f"---------------------- User Data to Delete ------------")
                print(f"   User ID   : {self.created_user_id}")
                if self.updated_user_data:
                    print(f"   Name      : {self.updated_user_data['name']}")
                    print(f"   Job       : {self.updated_user_data['job']}")
                else:
                    print(f"   Name      : {self.original_user_data['name']}")
                    print(f"   Job       : {self.original_user_data['job']}")
                ...
                assert response_body == "", f"Expected empty response body, got: {response_body}"
                        
                print(f"\n---------------------- Output ------------------------")
                print(f"✅ Success delete user data, USER ID = {self.created_user_id} \n")
    ...
    def test_complete_flow(self):
        ...
        print("\n TEST CREATE, UPDATE, THEN DELETE USER DATA \n")
        ...
        delete_success = self.test_delete_user()

        if create_success and update_success and delete_success:
            print(f"✅ Successfully create -> update -> and delete user data with ID =  {self.created_user_id}")
        else:
            ...
            if not delete_success:
                print("   ❌ Delete user data is failed")
